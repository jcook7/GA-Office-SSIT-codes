...
%                        'a1',0.4;'r1',0.04;'r2',0.1});
  ...
  simple_Model.fittingOptions.modelVarsToFit = 1:8;
...
%   for i=1:5
  ...
  simple_Model.parameters(simple_Model.fittingOptions.modelVarsToFit,2) = num2cell(simple_Model.maximizeLikelihood([simple_Model.parameters{simple_Model.fittingOptions.modelVarsToFit,2}],fitOptions));
%   end
  simple_Model.makeFitPlot;

%% Metropolis Hastings to Quantify Parameter Uncertainty
simple_Model.fittingOptions.modelVarsToFit = 1:4;
%   for i =1:5
...
%% Calculate CME Sensitivity to Parameter Variations
  ...
  simple_Model.makePlot(sensSoln,'marginals',simple_Model.tSpan(4))
...
%% save data
  save('simple_dusp1_model.mat','simple_Model')
  save('simple_dusp1_sens.mat','sensSoln')
  save('simple_dusp1_mhast.mat','mhResults')

%% Optimize Experiment Design (Same Number of Cells and Timepoints)
  nTotal = sum(simple_Model.dataSet.nCells);
  nCellsOpt = Model.optimizeCellCounts(fims,nTotal,'TR[1:4]');
  fimOpt = Model.evaluateExperiment(fims,nCellsOpt);
  Model.plotMHResults(mhResults,{FIM,fimOpt{1:4,1:4}});
%%
  close all
