...
tmpModel.tSpan = linspace(starttime,0,1000); 
...
Model.ssaOptions.Nexp = 100; 
Model.ssaOptions.nSimsPerExpt = 100;
Model.solve([],'simple_model_testB.csv'); 
Model.tSpan = Model.tSpan(2:end);

%% Associate Datasets with FSP Models.
Model.solutionScheme = 'FSP';  % Set solution scheme back to FSP.
%Model = Model.loadData('simple_model_testB.csv',{'x1','exp10_s1';'x2','exp10_s2'});
Model = Model.loadData('simple_model_testB.csv',{'x2','exp1_s2'});

Model.tSpan = Model.tSpan(2:end);
Model.initialTime = 0;
Model.fittingOptions.timesToFit = [false,ones(1,length(Model.tSpan)-1,'logical')];
Model.makeFitPlot
...
parfor iExp = 1:Model.ssaOptions.Nexp
    iExp
    %tmpModel = Model.loadData('simple_model_testB.csv',{'x1',['exp',num2str(iExp),'_s1'];'x2',['exp',num2str(iExp),'_s2']}); % Link non-distorted data.
    tmpModel = Model.loadData('simple_model_testB.csv',{'x2',['exp',num2str(iExp),'_s2']}); % Link non-distorted data.

    tmpModel.fittingOptions.timesToFit = [false,ones(1,length(Model.tSpan),'logical')];
    tmpModel.tSpan = Model.tSpan;
...
figure()
Model.makeMleFimPlot(squeeze(MLE(1,:,1:20)),fimFreePars,[1,3],0.95)
